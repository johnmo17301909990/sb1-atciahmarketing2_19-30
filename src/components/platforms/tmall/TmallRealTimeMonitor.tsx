...
import { useTmallRealtime } from '../../../hooks/useTmallRealtime';
...
export function TmallRealTimeMonitor({ dateRange }: Props) {
  const { data, loading, error } = useTmallRealtime('tmall');
      ...
      <div className="bg-white rounded-lg shadow p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div className="bg-blue-50 rounded-lg p-4">
            ...
            <p className="text-2xl font-bold text-blue-600 mt-2">{formatters.number(data.currentOnline)}</p>
          </div>
          ...
          <div className="bg-purple-50 rounded-lg p-4">
            ...
            <p className="text-2xl font-bold text-purple-600 mt-2">{formatters.number(data.todayOrders)}</p>
          </div>
        </div>
        ...
        <div className="h-[300px]">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={data.timeSeriesData}>
              ...
              <Tooltip />
              <Legend />
              <Line yAxisId="left" type="monotone" dataKey="onlineUsers" name="在线人数" stroke="#3B82F6" />
